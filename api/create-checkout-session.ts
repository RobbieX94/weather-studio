...
const PRICE_MAP: Record<string, string> = {
  ...
  freelancepro_monthly: process.env.STRIPE_PRICE_FREELANCEPRO_MONTHLY!,
  freelancepro_yearly: process.env.STRIPE_PRICE_FREELANCEPRO_YEARLY!,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!priceId) {
      return res.status(400).json({ error: `Combinación inválida: ${plan}_${billing}` })
    }
