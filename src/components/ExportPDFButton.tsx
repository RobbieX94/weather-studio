...
async function fetchHourly(project: Project, date: string): Promise<HourlySlot[]> {
  ...
  if (!TOMORROW_KEY) throw new Error('VITE_TOMORROW_API_KEY no configurada')
  ...
  const iconMap: Record<number, string> = {
    ...
    5000: '❄️', 6000: '🌨️', 8000: '⛈️',
  }
  const descMap: Record<number, string> = {
    1000: 'Despejado', 1001: 'Nublado', 1100: 'Mayormente despejado',
    1101: 'Parcialmente nublado', 1102: 'Mayormente nublado', 2000: 'Niebla',
    4000: 'Llovizna', 4001: 'Lluvia', 4200: 'Lluvia ligera', 4201: 'Lluvia intensa',
    5000: 'Nieve', 6000: 'Lluvia helada', 8000: 'Tormenta',
  }

  return intervals.map((iv: any) => {
    const v = iv.values
    const hour = new Date(iv.startTime).toLocaleTimeString('es-ES', {
      hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Madrid',
    })
    const riesgo: 'bajo' | 'medio' | 'alto' =
      v.precipitationProbability > 70 || v.windSpeed > 60
        ? 'alto'
        : v.precipitationProbability > 40 || v.windSpeed > 35
        ? 'medio'
        : 'bajo'
    return {
      time: hour,
      ...
      precipitation: Math.round(v.precipitationProbability),
      windSpeed: Math.round(v.windSpeed * 3.6),
      windDir: Math.round(v.windDirection),
...
async function generateHourlyNarrative(
  ...
  const buildFallback = () =>
    hours.map(h => {
      const windDesc =
        h.windSpeed <= 10 ? 'vientos muy flojos' :
        h.windSpeed <= 35 ? 'vientos flojos' :
        h.windSpeed <= 60 ? 'vientos moderados' : 'vientos fuertes'
      const lightDesc =
        h.uvIndex === 0 ? 'Sin radiación UV' :
        h.uvIndex <= 2 ? 'Valores bajos de luz' :
        h.uvIndex <= 5 ? 'Valores medios de luz' :
        h.uvIndex <= 7 ? 'Valores altos de luz' : 'Valores muy altos de luz'
      const rainDesc =
        h.precipitation === 0
          ? 'No se espera precipitación en esta hora.'
          : `Probabilidad de precipitación del ${h.precipitation}%.`
      const [hh] = h.time.split(':').map(Number)
      const nextHH = ((hh + 1) % 24).toString().padStart(2, '0')
      return `De ${h.time}h a ${nextHH}:00h: ${h.description}. ${rainDesc} ${lightDesc}. ${windDesc} (${h.windSpeed} km/h). Temperaturas en ${h.temp}°C (sensación ${h.feelsLike}°C).`
    })

  if (!GEMINI_KEY) {
    console.warn('[Gemini] VITE_GEMINI_API_KEY no definida — usando fallback numérico')
...
Usa lenguaje natural y profesional en castellano. Para el viento usa: "vientos muy flojos", "vientos flojos", "vientos moderados", "vientos fuertes". Para la dirección usa puntos cardinales en español. Para la luz usa: "Sin radiación UV", "Valores bajos de luz", "Valores medios de luz", "Valores altos de luz", "Valores muy altos de luz", "Radiación UV extrema". Si no se espera lluvia di "No se espera precipitación en esta hora.".
...
async function generatePDF5Days(project: Project, ai: AiAnalysis): Promise<void> {
  ...
  const infoBox = (text: string, bg: string, border: string, title?: string) => {
    ...
    if (title) {
      st(border); doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.text(title.toUpperCase(), M + 6, y + 6)
      st(C.text); doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.text(lines, M + 6, y + 12)
    } else {
      st(C.text); doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.text(lines, M + 6, y + 6)
    }
    y += h + 6
  }
...
async function generatePDFHourly(project: Project, date: string, hours: HourlySlot[]): Promise<void> {
  ...
  const meta: [string, string][] = [
    ...
    ['Análisis', narrative.length > 0 && narrative[0].startsWith('De ') ? 'Narrativa generada por Gemini 1.5 Pro' : 'Datos numéricos (Gemini no disponible)'],
    ['Generado', new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })],
  ]
  meta.forEach(([label, val]) => {
    st('#7a9bbf'); doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.text(label + ':', M, y)
  ...
  // Página 2: narrativa hora a hora
  doc.addPage(); y = M; sf('#ffffff'); doc.rect(0, 0, W, 297, 'F')
  ...
  if (narrative.length === 0) {
    st(C.muted); doc.setFont('helvetica', 'normal'); doc.setFontSize(10)
    doc.text('No se pudo generar la narrativa para este periodo.', M, y)
    y += 12
  }
  ...
  hours.forEach((h, hi) => {
    ...
    row.forEach((v, i) => { st(i === 0 ? '#0e7490' : C.text); doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.text(String(v), cx, y + 4.5); cx += cWidths[i] })
    y += 6
  })
    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      {loadingHr && hrStatus && (
        <div style={{
          ...
          borderRadius: 8, padding: '6px 12px', fontSize: 12, color: '#06b6d4',
      ...
      <button
        ...
        style={{
          ...
          color: '#7ab8ff', fontSize: 13, fontWeight: 700,
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              display: 'block', width: '100%', textAlign: 'left',
              padding: '10px 14px', borderRadius: 8, background: 'none',
              cursor: canHourly ? 'pointer' : 'not-allowed', border: 'none',
